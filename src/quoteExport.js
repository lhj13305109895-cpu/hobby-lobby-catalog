...
function makeQuoteXlsx({ entries, quoteNo, dateDisplay, imageData }) {
  ...
  const drawingAnchors = imageData.map((image, index) => {
    ...
    // A two-cell anchor stretches every image to the cell's aspect ratio.  The
    // picture source is slightly wider than its Excel cell, so that visibly
    // distorts tall pots. Use an explicit, centered one-cell frame instead.
    const cellWidth = 134 * 9525;
    const cellHeight = 105 * 12700;
    const padding = 4 * 9525;
    ...
    const colOffset = Math.round((cellWidth - width) / 2);
    const rowOffset = Math.round((cellHeight - height) / 2);
    return `<xdr:oneCellAnchor><xdr:from><xdr:col>1</xdr:col><xdr:colOff>${colOffset}</xdr:colOff><xdr:row>${rowZero}</xdr:row><xdr:rowOff>${rowOffset}</xdr:rowOff></xdr:from><xdr:ext cx="${width}" cy="${height}"/><xdr:pic><xdr:nvPicPr><xdr:cNvPr id="${index + 1}" name="Product ${index + 1}" descr="Thermal pot product image"/><xdr:cNvPicPr><a:picLocks noChangeAspect="1"/></xdr:cNvPicPr></xdr:nvPicPr><xdr:blipFill><a:blip r:embed="rId${index + 1}" cstate="print"/><a:stretch><a:fillRect/></a:stretch></xdr:blipFill><xdr:spPr><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></xdr:spPr></xdr:pic><xdr:clientData/></xdr:oneCellAnchor>`;
  }).join("");
  ...
  const imageByPattern = new Map(await Promise.all(uniquePatterns.map(async (pattern) => {
    ...
    if (!quoteImageCache.has(source)) {
      quoteImageCache.set(source, imageToDataUri(source)
        .then((dataUri) => prepareQuoteImage(dataUri, 145 / 136, pattern.family === "混色套装系列"))
